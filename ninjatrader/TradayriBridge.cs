...
// ===========================================================================
//  TradayriBridge - NinjaScript-AddOn: schickt geschlossene Bars an Tradayri
// ===========================================================================
//
//  EINBAUEN
//    1. Diese Datei nach
...
//    5. In Tradayri:  python scripts/run_paper.py --symbol MNQ --feed nt8
//
...
//     hat. Das ist Architektur-Invariante 1 von Tradayri: analysiert wird nur
...
{
    public class TradayriBridge : AddOnBase
    {
        ...
        private void StartServer()
        {
            ...
            catch (SocketException error)
            {
                ...
                NinjaTrader.Code.Output.Process(
                    "TradayriBridge: Port " + Port + " ist belegt (" + error.Message
            ...
            acceptThread = new Thread(AcceptLoop) { IsBackground = true, Name = "Tradayri-accept" };
            acceptThread.Start();

            heartbeatThread = new Thread(HeartbeatLoop) { IsBackground = true, Name = "Tradayri-beat" };
            heartbeatThread.Start();

            sendThread = new Thread(SendLoop) { IsBackground = true, Name = "Tradayri-send" };
            sendThread.Start();

            NinjaTrader.Code.Output.Process(
                "TradayriBridge lauscht auf 127.0.0.1:" + Port, PrintTo.OutputTab1);
        }
        ...
        private void HandleCommand(string line)
        {
            ...
            if (type == "order_modify") { ModifyOrder(line); return; }
            if (type == "flatten")
            {
        ...
        private void History(string symbol, string timeframe, long fromNs, long toNs)
        {
            ...
            // nicht - Tradayri haelt seine Historie selbst. Aber ohne ihn laesst
        ...
        private void Subscribe(string symbol, string timeframe)
        {
            ...
            request.Request((completed, error, message) =>
            {
                ...
                lock (requestLock)
                {
                    ...
                    // Beim Start NICHT die ganze Historie senden: Tradayri holt
                ...
                NinjaTrader.Code.Output.Process(
                    "TradayriBridge abonniert " + key, PrintTo.OutputTab1);
            });
        }
        ...
        private Instrument ResolveInstrument(string symbol)
        {
            ...
            //
            // Tradayri kennt nur das Wurzelsymbol "MNQ" und will es ueber
            ...
            // damit automatisch mit, ohne dass Tradayri etwas davon wissen muss.
        ...
        // laufenden Bars gesendet. Was Tradayri daraus zur Anzeige zusammensetzt,
        ...
        private void AttachTicks(string subscriptionKey, string symbol, Instrument instrument)
        {
            ...
            lock (tickLock)
            {
                ...
                catch (Exception error)
                {
                    ...
                    NinjaTrader.Code.Output.Process(
                        "TradayriBridge: keine Marktdaten fuer " + instrumentKey
        ...
        {
            // NinjaTrader stempelt eine Minutenbar auf ihr ENDE, Tradayri auf
        ...
        private void HeartbeatLoop()
        {
            while (running)
            {
                ...
                // Tradayri zwischen zwei Bars ueber die Verbindung bekommt.
        ...
        private void SubmitOrder(string line)
        {
            ...
            if (kind == "STOP") orderType = OrderType.StopMarket;
            ...
            if (orderType == OrderType.StopMarket && stopPrice <= 0)
            {
            ...
            try
            {
                Order entry = account.CreateOrder(
                    ...
                    orderType == OrderType.StopMarket ? stopPrice : 0,
                    string.Empty,
                    OrderRef(key, "entry"),
                ...
                // interne Merkposten: sie muessen auch dann wirken, wenn Tradayri
        ...
        private void ModifyOrder(string line)
        {
            string key = ExtractString(line, "order_key");
            ...
            foreach (Order order in orders)
            {
                ...
                {
                    if (order.Name.EndsWith("-target") && limitPrice > 0)
                    {
                        order.LimitPriceChanged = limitPrice;
                    }
                    else if (order.Name.EndsWith("-stop") && stopPrice > 0)
                    {
                        order.StopPriceChanged = stopPrice;
                    }
                    else if (order.Name.EndsWith("-entry"))
                    {
                        if (order.OrderType == OrderType.Limit && limitPrice > 0) order.LimitPriceChanged = limitPrice;
                        if (order.OrderType == OrderType.StopMarket && stopPrice > 0) order.StopPriceChanged = stopPrice;
                    }
                }
            }
            
            try { if (tradingAccount != null) tradingAccount.Change(orders); } catch { }
        }
        ...
        // verworfene Fuellung erzeugt eine Position, die Tradayri nicht kennt.
        ...
        private void OnPositionUpdate(object sender, PositionEventArgs args)
        {
            ...
            // Vorzeichenbehaftet: negativ = short. Tradayri haelt diese Zahl
        ...
        private static string MapOrderState(OrderState state)
        {
            switch (state)
            {
                ...
                // Vier Zustaende auf `accepted`: fuer Tradayri ist die Frage
                ...
                // trotzdem als `cancelled`: Tradayri nimmt auf eine Order, deren
        ...
        // `order_ref` aus Tradayri/broker/types.py, spiegelbildlich. Der
        ...
        // nach einem Neustart kennt Tradayri sie nicht mehr.
        ...
        /// ist: Tradayri soll `MNQ` sehen, nicht den Kontrakt des Monats.
        /// </remarks>
